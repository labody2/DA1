...
<div class="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <?php
            ...
            if (isset($_GET['categoryId'])) {
                ...
                // Gọi hàm getProductbyCategoryId với categoryId nhận từ URL
                $products = getProductbyCategoryId($conn, $categoryId);
            } else {
                // Nếu không có categoryId được chọn, hiển thị tất cả sản phẩm
                $products = getProducts($conn);
            }
            if (count($products) === 0) {
                echo '<p class="text-center text-red-500 text-lg font-semibold mt-8">Không có sản phẩm nào.</p>';
            }
        ...
        <div class="relative mx-auto w-full" >
            <a href="../page/index.php?link=BĐS_detail&&id=<?=$product["id"]?>" class="relative inline-block w-full transform transition-transform duration-300 ease-in-out hover:-translate-y-2">
